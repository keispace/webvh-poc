...
Next.js 앱 실행 후 다음 주소를 엽니다.

- [http://127.0.0.1:3000/](http://127.0.0.1:3000/) — DID, VC, VP, rotation 단계별 Console
- [http://127.0.0.1:3000/api-doc](http://127.0.0.1:3000/api-doc) — API 설명, 요청/응답 예제, 요청 편집 및 직접 호출
...
기본 주소는 `http://127.0.0.1:3000`입니다. `npm run demo`는 프로세스 내부 API router로 전체 흐름을 실행하고 실제 JWT와 검증 결과를 출력합니다.
...
Container는 `0.0.0.0:3000`에서 수신합니다. 실행 후 `http://127.0.0.1:3000`을 엽니다. `-p 3000:3000`의 첫 번째 `3000`은 host port이므로 `-p 8080:3000`으로 실행하면 `http://127.0.0.1:8080`에서 접근할 수 있습니다.

Issuer/holder private state는 container의 `/tmp/webvh-poc/webvh-poc.vercel.app`에 기록됩니다. 일회성 PoC 상태이므로 `--rm`으로 container가 제거되면 함께 제거됩니다.
...
Next.js가 listener port를 처리합니다. 실행·배포 전 `src/config.ts`의 `settings` 상수를 수정합니다.

- `didDomain`: DID에 들어갈 DNS hostname. 배포 도메인을 `https://` 없이 넣습니다.
- `dataDir`: private key와 DID log 저장 위치. PoC 기본값은 `/tmp/webvh-poc/webvh-poc.vercel.app`입니다.
- `verifierAudience`: VP audience exact match. 보통 `https://<didDomain>/verifier` 형식입니다.
- `issuerSlug`, `challengeTtlSeconds`, `vcTtlSeconds`: PoC 실행 설정입니다.
...
PoC는 Vercel production hostname인 `webvh-poc.vercel.app`으로 설정되어 있습니다.
